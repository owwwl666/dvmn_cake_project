...
def find_out_delivery_time(call):      
    if call.data == 'Узнать сроки доставки 🕒':
        ...
        bot.edit_message_text(chat_id=call.chat.id, message_id=call.message_id, text='вы вернулись в главное меню', reply_markup=get_text_messages())
...
def default_cake(message):
    ...
    btn3 = types.InlineKeyboardButton(callback_data=f'Выбран сироп Карамельный сироп', text='Карамельный сироп (+180 р.)')
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('Выбрать из тортов выше'))
    ...
    message = bot.send_message(message.from_user.id, 'Хотите сделать надпись на торте?   Мы можем разместить на торте любую надпись, например: «С днем рождения!» Введите текст надписи', reply_markup=markup)   
    bot.register_next_step_handler(message, address)
...
def address(message):
    ...
    btn1 = types.InlineKeyboardButton(callback_data=f'Детали заказа', text='Пропустить')
        ...
        btn5 = types.InlineKeyboardButton(callback_data=f'Вернуться в главное меню ⬅️', text='Вернуться в главное меню ⬅️')
        markup.add(btn1, btn2, btn3, btn4, btn5)
        bot.send_message(message.from_user.id, f'Введите адрес доставки\nСамовывоз возможен с завтрашнего дня с 9:00 до 17:00.', reply_markup=markup)
